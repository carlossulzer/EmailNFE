...
namespace Pop3
{
	...
	public class Pop3Parse
	{
		...
		{
            string retorno = Regex.Replace(line,"^[ |	]+filename=[\"]*([^\"]*).*$","$1").ToLower();

            if (retorno.ToLower().Contains(".xml") || retorno.ToLower().Contains(".eml") || retorno.ToLower().Contains(".zip"))
            {
                string[] nomes = retorno.Split('?');
