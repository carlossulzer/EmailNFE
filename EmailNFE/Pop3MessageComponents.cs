...
namespace Pop3
{
	...
	public class Pop3MessageComponents
	{
		...
		public Pop3MessageComponents(string[] lines, long startOfBody,string multipartBoundary, string mainContentType)
		{
			    ...
			    {				
				    ...
				    for(long i=startOfBody; i<stopOfBody;)
				    {
					    ...
					    while(i<stopOfBody)
					    {
						    ...
						    else if( Pop3Parse.GetSubHeaderLineType(line,boundary) ==	Pop3Parse.ComponetsDone )
						    {
							    emailComposed = true;
							    break;
						    }
                        ...
                        //    {
                                ...
                                try
                                {
                                    m_component.Add(new Pop3Component(contentType, name, filename, contentTransferEncoding, contentDescription, contentDisposition, data));
                                }
                                catch (Exception ex)
                                {
            ...
            //catch(Exception ex)
            //{
            //    throw new ApplicationException("Erro ao receber e-mail."+ex.Message.ToString());
            //}
		}
	}
}
